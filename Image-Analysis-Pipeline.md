...
After the image preprocessing phase, we began to focus on cell segmentation. Our approach combined the Otsu mask and color segmentation to isolate infected cells using thresholding techniques. Otsu’s method is a technique used to perform automatic thresholding, separating pixels from the foreground and background into two classes.[14] In order to segment cells from the background image, we applied Otsu segmentation on grayscaled Fig. 6. The resulting binary segmentation is seen in Fig. 7. Next, we overlaid the HSV intensities (Fig. 8) with our Otsu mask (Fig. 7) to isolate all cells and represent their respective color intensities in the HSV color space. To apply color thresholding, we first designed a purple mask representing the purple hues within our HSV image. Overlaying this purple mask seen in Fig. 9 onto our Otsu mask refined our image to the purple hues of our Otsu masked HSV image, creating Fig. 10. After calculating the mean purple intensity within our segmented cells (Fig. 8), we established a threshold by applying a data-informed factor to this average. This threshold improved the accuracy of our classification and allowed for unique segmentation based on average purple intensities for individual images.
</br>
